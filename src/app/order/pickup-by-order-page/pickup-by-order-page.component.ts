...
@Component({
  selector: 'app-pickup-by-order-page',
  templateUrl: './pickup-by-order-page.component.html',
  styleUrls: ['./pickup-by-order-page.component.scss']
})
export class PickupByOrderPageComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    private pickupByOrderSvc: PickupByOrderService,
    ...
    this.rx.select('route').pipe(takeUntil(this.onDestroy$)).subscribe((d: any) => {
      this.route = d;
      if (this.route && this.productGroups && this.route.length > 0 && this.productGroups.length > 0) {
        this.productGroups.sort((a, b) => {
          if (a.items.length > 0 && b.items.length > 0) {
            const aIndex = this.route.findIndex((r) => r.orderId === a.items[0].orderId);
            const bIndex = this.route.findIndex((r) => r.orderId === b.items[0].orderId);
      ...
      if (account) {
        ...
        //   self.merchants = rs.data;
          ...
          this.orderSvc.find(qOrder).pipe(takeUntil(this.onDestroy$)).subscribe((data) => {
            ...
            this.pickupByOrderSvc.find(qPickup).pipe(takeUntil(this.onDestroy$)).subscribe((r: any) => {
              const pickups = r.data;
              // const groups = this.groupByMerchants(orders);
              const productGroups = pickups;
              this.productGroups = productGroups;
              if (this.route.length > 0 && this.productGroups.length > 0) {
                this.productGroups.sort((a, b) => {
                  if (a.items.length > 0 && b.items.length > 0) {
                    const aIndex = this.route.findIndex((r) => r.orderId === a.items[0].orderId);
                    const bIndex = this.route.findIndex((r) => r.orderId === b.items[0].orderId);
  ...
  onChangePickup(event: any, row: any) {
    ...
    if (row._id) {
      ...
      delete data._id;
      this.pickupByOrderSvc.update(pickupId, data).pipe(takeUntil(this.onDestroy$)).subscribe(() => {
      ...
      delete row._id;
      const data = { ...row, status };
      this.pickupByOrderSvc.save(data).pipe(takeUntil(this.onDestroy$)).subscribe(() => {
        this.snackBar.open('', '取货状态已更改', { duration: 1000 });
        this.reload(this.pickup, this.deliverDate, OrderType.GROCERY).then((r: any) => {
          // this.orders = r.orders;
          // this.groups = r.groups;
